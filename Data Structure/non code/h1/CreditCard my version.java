...
public class CreditCard {
  ...
  public void makePayment(double amount) {            // make a payment
    if (amount >= 0) { balance -= amount; }   // if the balance is nonnegative, it will go through
    else { System.out.println("Error, cannot process a negative value as payment"); }  // other wise it fails
  }

  // MY NEW METHOD ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!!!!
  public void changeLimit() {   // method for changing the credit limit
    Scanner input = new Scanner (System.in);   // using the scanner function
    System.out.println("The current limit is: " + limit);
    System.out.println("Please enter the new limit: ");// it prompts for a new limit numbers
    limit = input.nextInt(); // update
  }
  ...
  public static void main(String[] args) {
    ...
    for (CreditCard card : wallet) {
      ...
      while (card.getBalance() > 200.0) {
        System.out.println("How much would you like to pay today: "); // ask for payment amount
        int payment = input.nextInt(); // input payment
        card.makePayment(payment);
        System.out.println("New balance = " + card.getBalance());
      }
    }
    wallet[0].getLimit();    // just set of statement for testing my change limit
    wallet[0].changeLimit();  // not much of a program, is it?
